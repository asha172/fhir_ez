...
const ResouceInfoList = ({ sourceData, onClick }) => {
  ...
  const generateTRTag = (key, tdTags, idOddRow) => {
    let dynamicTrStyle = !idOddRow
      ? { ...style.trStyle, backgroundColor: oddRowBGColor }
      : style.trStyle;

    let trItem = (
      <tr id={key} key={key} style={dynamicTrStyle} onClick={onClick}>
        {tdTags}
      </tr>
    );
  ...
  const generateTDTag = (
    ...
    tdTagItemList.push({ dataRow: dataSource, tdTags: tdTags });
    ...
    if (dataSource.children.length > 0) {
      ...
      for (var i = 0; i < dataSource.children.length; i++) {
        ...
        let tdTagItem = generateTDTag(
          dataSource.children[i],
          isParentLast,
          isLastNode
        );
        subTdTagTems = subTdTagTems.concat(tdTagItem);
      }
      tdTagItemList = tdTagItemList.concat(subTdTagTems);
    }

    return tdTagItemList;
  };

  const convertDataToTRTags = (
    sourceBodyDatas,
    isLastTRNode = false,
    isLastTDNode = false
  ) => {
    ...
    let trTagList = [];
    let tdList = generateTDTag(dataSource, isLastTRNode, isLastTDNode);

    // if(depth === 0){
    if (dataSource.children.length > 0) {
      for (let i = 0; i < tdList.length; i++) {
        let idKey = tdList[i].dataRow.id;
        let subTrTag = generateTRTag(idKey, tdList[i].tdTags, i % 2 === 0);
  ...
  return tableBodyTags;
};

export default ResouceInfoList;
